...
import { useStudentI18n } from '@/app/student/i18n';
  ...
  return (
    <header className="h-16 bg-[var(--bg-page)]/80 backdrop-blur-md border-b border-[var(--border)] sticky top-0 z-20 flex items-center justify-between px-4 md:px-8">
      ...
      <div className="hidden md:block">
        <h1 className="text-lg font-bold text-[var(--text-primary)]">{t('welcome')}, {session?.name?.split(' ')[0] || 'Student'}! 👋</h1>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <Link href="/student/notices" className="relative p-2 text-[var(--text-secondary)] hover:bg-[var(--bg-input)] rounded-full transition-colors">
