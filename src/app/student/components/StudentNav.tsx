    ...
    <>
      ...
      <aside className="hidden md:flex w-64 bg-[var(--bg-card)] border-r border-[var(--border)] h-screen flex-col fixed left-0 top-0">
        <div className="p-6 border-b border-[var(--border)]">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1 tracking-wider">Student Portal</p>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          <div className="pt-4 mt-4 border-t border-[var(--border)] space-y-2">
            <Link href="/student/notices" className="flex items-center gap-3 px-4 py-3 rounded text-[var(--text-secondary)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)] text-sm font-medium"><Bell className="w-5 h-5"/> Notices</Link>
            <Link href="/student/documents" className="flex items-center gap-3 px-4 py-3 rounded text-[var(--text-secondary)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)] text-sm font-medium"><FileText className="w-5 h-5"/> Documents</Link>
          </div>
        </nav>

        <div className="p-4 border-t border-[var(--border)] space-y-2">
          <Link href="/student/sos" className="block text-center w-full px-4 py-3 bg-[var(--danger)] text-white rounded font-bold shadow hover:bg-red-600 transition-colors">
            EMERGENCY SOS
          </Link>
          <button onClick={() => { clearSession(); window.location.href = '/student/login'; }} className="flex items-center gap-3 px-4 py-3 w-full text-left text-[var(--danger)] hover:bg-[var(--danger-bg)] rounded-[var(--radius-md,8px)] transition-colors font-medium text-sm">
