...
export default function ManagerTenantDetail() {
  ...
  useEffect(() => {
    if (id) {
      const t = api.tenants.getById ? api.tenants.getById(id) : api.tenants.listByProperty('all').find((t: any) => t.profile.id === id || t.user.id === id);
      setTenant(t);
      if (t) {
        setInvoices(tenantOperationsApi.getInvoices(t.user.id));
      }
    }
  }, [id]);

  const handleCheckout = () => {
    if (confirm('Are you sure you want to checkout this tenant? This will revoke their access and free their bed.')) {
      api.tenants.checkoutTenant(tenant.profile.id, user?.id || '');
      router.push('/manager/tenants');
    }
  };

  if (!tenant) return <div className="p-6 text-[var(--text-secondary)]">Loading...</div>;

  return (
    <div className="space-y-6 pb-20 max-w-4xl mx-auto">
      <Link href="/manager/tenants" className="inline-flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--primary)] text-sm font-medium transition-colors">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-xl,16px)] overflow-hidden shadow-sm">
        <div className="p-8 bg-[rgba(99,102,241,0.02)] border-b border-[var(--border)] flex items-center gap-6">
          <div className="w-20 h-20 rounded-full bg-[var(--primary-subtle)] border-2 border-[var(--primary)] flex items-center justify-center text-[var(--primary)] text-2xl font-bold">
            {tenant.user?.name?.charAt(0) || 'U'}
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">{tenant.user?.name || 'Unknown'}</h1>
            <div className="flex items-center gap-4 mt-2 text-sm text-[var(--text-secondary)]">
              <span className="flex items-center gap-1"><User className="w-4 h-4"/> ID: {tenant.profile.userId?.slice(0,6)}</span>
              <span className="flex items-center gap-1"><MapPin className="w-4 h-4"/> Bed: {tenant.profile.bedId || '-'}</span>
              <span className="flex items-center gap-1"><Calendar className="w-4 h-4"/> Joined: {new Date(tenant.profile.createdAt).toLocaleDateString()}</span>
            </div>
          </div>
          {tenant.profile.status !== 'checked_out' && (
            ...
            >
              <LogOut className="w-4 h-4" /> Checkout Tenant
            </button>
          )}
        </div>
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="font-medium text-[var(--text-primary)]">{tenant.user?.phone || '-'}</div>
              <div className="text-[var(--text-secondary)]">Email</div>
              <div className="font-medium text-[var(--text-primary)]">{tenant.user?.email || '-'}</div>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="font-medium text-[var(--text-primary)]">{tenant.profile.parentName || '-'}</div>
              <div className="text-[var(--text-secondary)]">Parent Phone</div>
              <div className="font-medium text-[var(--text-primary)]">{tenant.profile.parentPhone || '-'}</div>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="font-medium text-[var(--text-primary)] flex items-center gap-1"><IndianRupee className="w-3.5 h-3.5"/> {tenant.profile.rentAmount || 0}</div>
              <div className="text-[var(--text-secondary)]">Current Dues</div>
              <div className={`font-medium flex items-center gap-1 ${tenant.profile.duesAmount > 0 ? 'text-[var(--danger)]' : 'text-[var(--success)]'}`}>
                <IndianRupee className="w-3.5 h-3.5"/> {tenant.profile.duesAmount || 0}
              </div>
              ...
              <div className="font-medium text-[var(--text-primary)] flex items-center gap-1">
                <Utensils className="w-3.5 h-3.5"/> {tenant.profile.hasMessFacility ? 'Yes (Included)' : 'No'}
              </div>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className={`font-bold ${tenant.profile.pgScore >= 80 ? 'text-[var(--success)]' : 'text-[var(--warning)]'}`}>{tenant.profile.pgScore}/100</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-[var(--border)]">
          ...
          {tenant.profile.stayStartDate && tenant.profile.stayEndDate ? (
            <div className="space-y-4">
              <div className="flex gap-4 text-sm text-[var(--text-secondary)] mb-6 bg-[var(--bg-input)] p-4 rounded-lg">
                <div><strong>Start Date:</strong> {new Date(tenant.profile.stayStartDate).toLocaleDateString()}</div>
                <div><strong>End Date:</strong> {new Date(tenant.profile.stayEndDate).toLocaleDateString()}</div>
              </div>
