...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setNotices(tenantOperationsApi.getNotices(profile.propertyId).slice(0, 3));
    }
  }, [profile]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid ${profile.hasMessFacility ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}>
        {/* Dues */}
        <Link href="/tenant/rent" className="bg-gradient-to-br from-[var(--bg-card)] to-[var(--bg-page)] border border-[var(--border)] rounded-[var(--radius-lg,12px)] p-5 flex flex-col justify-between hover:border-[var(--primary)] hover:-translate-y-1 transition-all shadow-sm group relative overflow-hidden">
      ...
      {/* SOS Button */}
      <Link href="/tenant/sos" className="block w-full bg-[#7f1d1d] text-white text-center py-4 rounded-[var(--radius-lg,12px)] font-black text-lg shadow-lg hover:bg-[#991b1b] transition-all active:scale-95 flex items-center justify-center gap-2 border border-red-900/50">
      ...
      <div className={`grid ${profile.hasMessFacility ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-6`}>
        ...
        {profile.hasMessFacility && (
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-lg,12px)] p-5 shadow-sm">
            ...
            <Link href="/tenant/mess" className="block mt-4 text-center text-sm font-bold text-[var(--primary)] py-2 border border-[var(--primary)] rounded hover:bg-[var(--primary-subtle)] transition-colors">
              Manage Meals
            </Link>
          </div>
        )}
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-lg,12px)] p-5 shadow-sm">
          ...
          <Link href="/tenant/notices" className="inline-block mt-4 text-sm font-bold text-[var(--primary)] hover:underline">
            View All &rarr;
          </Link>
        </div>
      </div>
            ...
            <form onSubmit={(e) => {
              ...
              import('@/lib/api').then(({ api }) => {
                api.managerEnquiries.create({
                  ...
                  name,
                  phone,
                  referredByTenantId: profile.userId,
                  notes: `Referred by existing tenant: ${profile.user?.name || 'Friend'} (Room: ${profile.roomNumber})`
                });
