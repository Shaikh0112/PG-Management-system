...
export default function TenantSosPage() {
  ...
  const handleSos = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      tenantOperationsApi.triggerSos(profile.tenantId || profile.userId, profile.propertyId, session.id);
      setSuccess(true);
      setLoading(false);
    }, 1000);
  };
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6 animate-in fade-in zoom-in duration-500">
        ...
        <button onClick={() => router.push('/tenant/dashboard')} className="mt-8 px-8 py-3 bg-[var(--bg-input)] text-[var(--text-primary)] rounded font-bold hover:bg-[var(--primary)] hover:text-white transition-colors">
          Return to Dashboard
        </button>
      </div>
    );
